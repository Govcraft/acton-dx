...
mod tests {
    use super::*;
    use crate::jobs::agent::queue::QueuedJob;
    use chrono::Utc;
    ...
    async fn test_spawn_redis_agent() {
        ...
        assert!(handle.is_ok(), "Failed to spawn Redis persistence agent");
    }
    ...
    async fn test_persist_job() {
        ...
        let job = QueuedJob {
            ...
            payload: serde_json::to_vec(&serde_json::json!({"test": "data"})).unwrap(),
            priority: 10,
            max_retries: 3,
        ...
        // Send persist message (fire-and-forget)
        handle.send(PersistJob { job }).await;

        // Give it time to process
        tokio::time::sleep(Duration::from_millis(100)).await;

        // Note: Full verification would require querying Redis
        // This test just ensures the message is sent without panic
    }
    ...
    async fn test_mark_completed() {
        ...
        // Send mark completed message
        handle.send(MarkJobCompleted {
            id: job_id,
    ...
    async fn test_concurrent_operations() {
        ...
        for i in 0..10 {
            ...
            handles.push(tokio::spawn(async move {
                let job = QueuedJob {
                    id: JobId::new(),
                    job_type: format!("test_job_{i}"),
                    payload: serde_json::to_vec(&serde_json::json!({"index": i})).unwrap(),
                    priority: 10,
                    max_retries: 3,
                    timeout: Duration::from_secs(60),
                    enqueued_at: Utc::now(),
        ...
        // Concurrent operations should complete without issues
    }
}
